...
- **Runtime `.mural` compilation exists.** `instantiate(source: string, ctx: Record<string, unknown>, options?): unknown` from `@pragmatic-tech-ai/mural/compiler` compiles `.mural` markup text in-process (browser-safe, no `node:fs`). A `Fragment{…}` root compiles to a **factory** `(data) => Visual`; `ctx` must supply the runtime symbols (`{ ...runtime, ...basic, ...visualEngine }`).
