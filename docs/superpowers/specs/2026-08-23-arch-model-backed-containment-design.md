...
**Tech stack:** TypeScript. Plexus (Electron renderer, `src/renderer/src/modules/architecture-projects`), consuming Mural from Verdaccio. TODL model layer (`@pragmatic-lab/todl`). Mural framework (`@pragmatic-lab/mural`) — requires the sub-project-1 container primitive **plus** a new VM-opt-in-container seam, published to Verdaccio and consumed by a Plexus bump.
