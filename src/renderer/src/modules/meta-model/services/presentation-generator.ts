...
// Emit the presentation ASSET dictionary: one geometry per distinct icon, the
// author `merge` block (author templates live in presentation/*.mu), and a
// reserved seam for future base64 embedded content. No DataTemplates — entity/
// class visuals are author-owned (scaffolded once; see presentation-scaffold.ts).
// Deterministic: icons sorted; `authorOverrideDicts` are compiled-dictionary
// identifiers to `merge` (author keys win), [] emits no merge block. `dictName`
// is the `resources <Name>` block identifier — kept per-domain for the compiled-
