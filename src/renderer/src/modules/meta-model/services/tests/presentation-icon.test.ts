...
// The colored icon is BAKED into the assets artifact at compile (svgToIconJs, via the
// `include colored` keyword), keyed by its resource key, so it resolves standalone with
// no SVG file present. Guards the icon end to end: publish → load → the resource key
// resolves to a real IconDefinition (the IconKeyConverter then draws it in the one
// default template).
